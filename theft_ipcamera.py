...
def main(_argv):
    ...
    stream_url = f"rtsp://{ip}:{port}/h264_ulaw.sdp"
    if username and password:
        stream_url = f"rtsp://{username}:{password}@{ip}:{port}/h264_ulaw.sdp"
    ...
    cap = VideoStream(resolution=args.get("res"), cam=stream_url).start()
